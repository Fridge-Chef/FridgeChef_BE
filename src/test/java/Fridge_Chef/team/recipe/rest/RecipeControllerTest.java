        ...
        actions.andExpect(status().isOk())
                .andDo(document("추천 레시피 검색",
                        ...
                        responseFields(
                                ...
                                fieldWithPath("content[].userName").description("레시피 작성자").type(JsonFieldType.STRING),
                                fieldWithPath("content[].mainImage").description("레시피 이미지").type(JsonFieldType.STRING),
                                ...
                                fieldWithPath("content[].myHit").description("내가 조회한 여부").type(JsonFieldType.BOOLEAN),
                                fieldWithPath("content[].myMe").description("내가 작성한 레시피 여부").type(JsonFieldType.BOOLEAN),
                                fieldWithPath("content[].click").description("클릭 수").type(JsonFieldType.NUMBER),
                                fieldWithPath("content[].createTime").description("작성일").type(JsonFieldType.STRING),
                                fieldWithPath("content[].have").description("사용 가능한 재료 수").type(JsonFieldType.NUMBER),
                                fieldWithPath("content[].withoutCount").description("제외된 재료 수").type(JsonFieldType.NUMBER),
                                fieldWithPath("content[].without").description("제외된 재료 목록").type(JsonFieldType.ARRAY),
